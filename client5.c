...
int main(int argc, char *argv[]) 
{
    ...
    */
    ...
    {
        perror("connect");
        close(socketClient); // on ferme la ressource pour quitter
        exit(-2);
    }

    printf("Connection reussi avec the server \n");

    char* pseudo = (char*) malloc(256*sizeof(char));

    printf("Veuillez choisir un pseudo s'il vous plaît avant d'envoyer un message : \n");

    scanf("%s", pseudo);

    int taille = sizeof(pseudo);

    if(send(socketClient,(void *) pseudo, taille,0) == -1){
        perror("Une erreur a été commise");
    }
